<!-- AUTO-GENERATED — do not edit. Source: https://github.com/jhnnsrs/dokuments-server/blob/next/CONFIG.md. Run scripts/sync_config_docs.py to update. -->

# Dokuments — Configuration Reference

This document explains how the **dokuments** service is configured, then lists every
...
[`dokuments_server/configuration.py`](https://github.com/jhnnsrs/dokuments-server/blob/next/dokuments_server/configuration.py); this file
...
3. **The YAML file** — [`config.yaml`](https://github.com/jhnnsrs/dokuments-server/blob/next/config.yaml) by default.
...
```bash
ARKITEKT_CONFIG_FILE=/etc/dokuments/config.yaml python manage.py runserver
```
...
| `datalayer.access_key` | `DATALAYER__ACCESS_KEY` |

Lists and nested objects (e.g. `authentikate.issuers`, `datalayer.media`) are awkward to
...
top-level `s3` block). `access_key`, `secret_key`, and the `media` bucket are required
for this service.
